...
class ReadBarrierMarkAndUpdateFieldSlowPathRISCV64 : public SlowPathCodeRISCV64 {
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) override {
    ...
    __ Move(tmp, value);
    __ Sc(tmp, tmp_ptr);
    __ Beqzc(tmp, &loop_head);   // return in tmp 0: success, 1 : fail
    __ Bind(&exit_loop);
...
Location CriticalNativeCallingConventionVisitorRiscv64::GetNextLocation(DataType::Type type) {
  ...
  if (DataType::IsFloatingPointType(type)) {
    ...
    if (gpr_index_ < 1u + kParameterCoreRegistersLength) {
      location = Location::RegisterLocation(gpr_index_ == 0u ? A0 : kParameterCoreRegisters[gpr_index_ - 1u]);
      ++gpr_index_;
    }
  }
...
Location CriticalNativeCallingConventionVisitorRiscv64::GetReturnLocation(DataType::Type type) const {
...
void CodeGeneratorRISCV64::LoadBootImageAddress(GpuRegister reg, uint32_t boot_image_reference) {
  if (GetCompilerOptions().IsBootImage()) {
    ...
    __ LoadLiteral(reg, kLoadUnsignedWord, DeduplicateBootImageAddressLiteral(address));
  }
}

void CodeGeneratorRISCV64::LoadTypeForBootImageIntrinsic(GpuRegister reg,
...
void CodeGeneratorRISCV64::LoadIntrinsicDeclaringClass(GpuRegister reg, HInvoke* invoke) {
...
void CodeGeneratorRISCV64::LoadClassRootForIntrinsic(GpuRegister reg,
...
void InstructionCodeGeneratorRISCV64::VisitPredicatedInstanceFieldGet(
  ...
  Riscv64Label finish;
  LocationSummary* locations = instruction->GetLocations();
  GpuRegister target = locations->InAt(1).AsRegister<GpuRegister>();
  ...
  if (read_barrier_option == kWithReadBarrier) {
    ...
    if (kUseBakerReadBarrier) {
      ...
      if (kBakerReadBarrierThunksEnableForGcRoots) {
        ...
        int16_t offset_high = High20Bits(offset - offset_low);  // Accounts for sign
                                                                // extension in lwu.
        bool short_offset = IsInt<11>(static_cast<int32_t>(offset));
...
void InstructionCodeGeneratorRISCV64::VisitInvokeInterface(HInvokeInterface* invoke) {
  ...
  if (invoke->GetHiddenArgumentLoadKind() != MethodLoadKind::kRecursive &&
    ...
    Location hidden_reg = invoke->GetLocations()->GetTemp(1);
    ...
    DCHECK_EQ(T0, hidden_reg.AsRegister<GpuRegister>());
  ...
  if (invoke->GetHiddenArgumentLoadKind() == MethodLoadKind::kRuntimeCall) {
    ...
    Location hidden_reg = invoke->GetLocations()->GetTemp(1);
    DCHECK_EQ(T0, hidden_reg.AsRegister<GpuRegister>());
    __ Move(hidden_reg.AsRegister<GpuRegister>(), temp);
  }
...
void CodeGeneratorRISCV64::LoadMethod(MethodLoadKind load_kind, Location temp, HInvoke* invoke) {
  switch (load_kind) {
    ...
    case MethodLoadKind::kBssEntry: {
      ...
      PcRelativePatchInfo* info_low = NewMethodBssEntryPatch(invoke->GetMethodReference(), info_high);
      EmitPcRelativeAddressPlaceholderHigh(info_high, AT, info_low);
  ...
  switch (load_kind) {
    ...
    case HLoadClass::LoadKind::kBssEntryPackage: {
      CodeGeneratorRISCV64::PcRelativePatchInfo* bss_info_high =
          codegen_->NewTypeBssEntryPatch(cls, cls->GetDexFile(), cls->GetTypeIndex());
      CodeGeneratorRISCV64::PcRelativePatchInfo* info_low =
          codegen_->NewTypeBssEntryPatch(cls,cls->GetDexFile(), cls->GetTypeIndex(), bss_info_high);
...
void InstructionCodeGeneratorRISCV64::GenerateMinMaxInt(LocationSummary* locations, bool is_min) {
  ...
  if (lhs == rhs) {
    ...
    if (lhs == out) {
      if (is_min) {
        __ Bltc(lhs, rhs, &label);
      } else {
        __ Bltc(rhs, lhs, &label);
      }
      __ Move(out, rhs);
    } else if (rhs == out) {
      if (is_min) {
